...
function getRating(watchList) {
  ...
  let christopherNolanMovies = watchList.filter(
    (movie) => movie['Director'] == 'Christopher Nolan'
  );

  let averageRating =
    christopherNolanMovies.reduce(
      (totalRating, movie) => totalRating + Number(movie['imdbRating']),
      0
    ) / christopherNolanMovies.length;
